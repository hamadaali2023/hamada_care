...
class DoctorAuthController extends Controller
{
            ...
            $token =  Auth::guard('doctor-api') -> attempt($credentials);
                if(!$token)
                    ...
                    if($UserData->is_activated ==0)
                    {
                        ...
                        // if(!$doctor_case){
                        //     // dd('da');
                        //     $doctor_case->doctor_not_status=1;
                        //     $doctor_case->doctor_servic_status=1;
                        // }
    ...
    public function personalDataUpdate(Request $request)
    {
        ...
        {  
            ...
            $file_nameone2 = time().rand(1,99). '.' . $request->file('personality_photo')->extension();
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                if($doctorss==null){
                    ...
                    $details = [
                        'title' => 'Password of Total Health',
                        'body' => 'Cope this password to enter Total Health: ' ." " . $gene . " "
                    ];
